...
from dropbear_rl_lab.assets.robots.dropbear import DROPBEAR_CFG as ROBOT_CFG
from dropbear_rl_lab.tasks.locomotion import mdp
...
class RobotSceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with a Dropbear humanoid robot."""

    # ground terrain - simplified for testing
    terrain = TerrainImporterCfg(
        prim_path="/World/ground",
        terrain_type="plane",  # Use simple plane for testing
        terrain_generator=None,  # No complex terrain
    ...
    height_scanner = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/world",  # Dropbear's main body
    ...
    # Contact sensors for feet/ankle detection
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        gait_phase = ObsTerm(func=mdp.gait_phase, params={"period": 0.8})  # Slower gait for humanoid
...
class RewardsCfg:
    ...
    joint_deviation_legs = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg(
                "robot", 
                joint_names=[
                    "PG_.*", "LL_.*", "RL_.*"  # All leg and pelvic joints
                ]
            )
        },
    )
    ...
    gait = RewTerm(
        ...
        weight=0.5,
        params={
            "period": 0.8,  # Slower humanoid gait
            "offset": [0.0, 0.5],  # Only 2 legs for humanoid (left, right)
            "threshold": 0.55,
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["LL_skateboard_bearing__20__1", "RL_skateboard_bearing_left_2"]),  # Available foot bodies
        },
    )
    ...
    feet_slide = RewTerm(
        ...
        weight=-0.2,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["LL_skateboard_bearing__20__1", "RL_skateboard_bearing_left_2"]),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["LL_skateboard_bearing__20__1", "RL_skateboard_bearing_left_2"]),
        },
    )
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "threshold": 1,
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["(?!.*skateboard_bearing.*).*"]),
        },
    )
...
class CurriculumCfg:
    ...
    # terrain_levels = CurrTerm(func=mdp.terrain_levels_vel)  # Disabled for plane terrain
    lin_vel_cmd_levels = CurrTerm(mdp.lin_vel_cmd_levels)


@configclass
class DropbearVelocityRoughEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # --- Robot Cfg ---
        self.gait_f = 1.25  # Slower gait frequency for humanoid
...
@configclass
class DropbearVelocityRoughEnvCfg_PLAY(DropbearVelocityRoughEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.num_envs = 32
        # Only modify terrain generator if it exists (not None for plane terrain)
        if self.scene.terrain.terrain_generator is not None:
